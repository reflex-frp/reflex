...
data MergeSubscribed k
   = MergeSubscribed { mergeSubscribedOccurrence :: !(IORefMaybe (DMap k Identity))
...
subscribeEventSubscribed es ws = case es of
  ...
  EventSubscribedFan k subscribed -> do
    ...
    modifyIORef' (fanSubscribedSubscribers subscribed) $ DMap.insertWith (liftA2 (++)) (FanSubscriberKey k) (Identity [ws]) --IMPORTANT: The [ws] is the first argument to (++) and any existing list is the second argument.  If this were not the case, this would be too slow.
