...
instance ReflexHost t => Adjustable t (PerformEventT t m) where
  ...
  traverseIntMapWithKeyWithAdjust f outerDm0 outerDm' = PerformEventT $ traverseIntMapWithKeyWithAdjustRequesterTWith (defaultAdjustIntBase traverseIntMapPatchWithKey) (\k v -> unPerformEventT $ f k v) (coerce outerDm0) (coerceEvent outerDm')
  traverseDMapWithKeyWithAdjust f outerDm0 outerDm' = PerformEventT $ traverseDMapWithKeyWithAdjustRequesterTWith (defaultAdjustBase traversePatchDMapWithKey) Dict mapPatchDMap weakenPatchDMapWith mergeMapIncremental coincidencePatchMap (\k v -> unPerformEventT $ f k v) (coerce outerDm0) (coerceEvent outerDm')
  traverseDMapWithKeyWithAdjustWithMove f outerDm0 outerDm' = PerformEventT $ traverseDMapWithKeyWithAdjustRequesterTWith (defaultAdjustBase traversePatchDMapWithMoveWithKey) Dict mapPatchDMapWithMove weakenPatchDMapWithMoveWith mergeMapIncrementalWithMove coincidencePatchMapWithMove (\k v -> unPerformEventT $ f k v) (coerce outerDm0) (coerceEvent outerDm')

defaultAdjustBase :: forall t v v2 k' p. (Monad (HostFrame t), ReflexHost t)
...
defaultAdjustIntBase :: forall t v v2 p. (Monad (HostFrame t), ReflexHost t)
...
hostPerformEventT :: forall t m a.
                  ...
                  => PerformEventT t m a
                  -> m (a, FireCommand t m)
hostPerformEventT a = runHostFrame $ do
  rec ((outerResult, e), fc) <- runRequesterT' $ \(eventToPerform :: Event t (Seq (RequestItem t x (HostFrame t) Identity))) -> do
        (response, responseTrigger) <- newEventWithTriggerRef
        result <- unPerformEventT a
        let f :: Seq (RequestItem t x (HostFrame t) Identity) -> m [DSum (EventTrigger t) Identity]
            f toPerform = do
              responses <- runHostFrame $ fmap (Seq.fromList . catMaybes . toList) $ forM toPerform $ \(RequestItem mt v) -> do
                x <- v
                return $ case mt of
                  Nothing -> Nothing
              ...
              pure $ case mrt of
                ...
                Nothing -> []
        return $ (,) response $ (,) (result, fmapCheap f eventToPerform) $ FireCommand $ \triggers (readPhase :: ReadPhase m a') -> do
          let go :: [DSum (EventTrigger t) Identity] -> m [a']
              go ts = do
                ...
                case mToPerform of
                  ...
                  Just toPerform -> do
                    followupEventTriggers <- toPerform
                    (result':) <$> go followupEventTriggers
          go triggers
      eventToPerformHandle <- subscribeEvent e
  return (outerResult, fc)
...
instance ReflexHost t => MonadMutate t (PerformEventT t m) where
  {-# INLINABLE mutateFanCell #-}
  mutateFanCell c a = PerformEventT $ mutateFanCell c a
